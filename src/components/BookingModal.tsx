...
import { X, Lock, Zap, Shield, Phone, Loader, AlertCircle } from 'lucide-react'
...
export default function BookingModal({ companion, onClose, onSuccess }: Props) {
  ...
  const startPolling = (bId: string, cId: string) => {
    let count = 0
    const interval = setInterval(async () => {
      ...
      try {
        const res = await fetch(`/api/mpesa/status?bookingId=${bId}&checkoutRequestId=${cId}`)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setStep('waiting')
      setPollCount(0)
      startPolling(data.bookingId, data.checkoutRequestId)
    } catch (err: any) {
